...
class LibreOfficeProcessManager:
    ...
    async def convert(self, command: ConversionCommand) -> Path:
        ...
        try:
            ...
            raise DocumentConversionError(f"Conversion failed: {exc}") from exc
    ...
    async def _run_conversion(
        self, process_id: int, command: ConversionCommand
    ) -> Path:
    ...
    async def _cleanup_loop(self) -> None:
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                logger.error("Error in cleanup loop", error=str(exc))
