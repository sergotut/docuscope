...
class DecisionRule(Protocol):
    ...
    def evaluate(
        ...
        *,
        ...
        Args:
            result: Доменный результат детекции типа документа.
            normalized: Нормализованные входные данные (каноничное расширение и MIME).
            warnings: Объединённые предупреждения нормализации и/или детектора.

        Returns:
            Код причины отклонения, если правило считает документ неподходящим;
            иначе None, если правило пройдено.
        """
        ...
